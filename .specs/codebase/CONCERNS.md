...
- Files: `finsight-backend/src/main/resources/application.properties` (`spring.jpa.hibernate.ddl-auto=update`); `finsight-backend/pom.xml` (no `flyway-core` dependency); README claim in `finsight-backend/README.md` ("Flyway — database migrations").
...
- Files: `finsight-backend/src/main/java/com/lcs/finsight/models/FinancialTransaction.java` (lines 33-34); set-but-unused in `finsight-backend/src/main/java/com/lcs/finsight/services/FinancialTransactionService.java` (`create` lines 101-102, `update` lines 126-127) — no logic reads them to generate occurrences.
...
- Files: `finsight-backend/.gitignore` (ignores `.env`, `.env.production`); `finsight-backend/.env.example` (placeholder keys only: `SPRING_DATASOURCE_*`, `JWT_SECRET_KEY`); secrets consumed via env in `application.properties`.
- Current mitigation: Real `.env` files are gitignored and a sanitized `.env.example` documents required keys. A live `finsight-backend/.env` exists on disk but is untracked. No secrets are hardcoded in source.
...
- Risk: `finsight-frontend/.env.development` and `.env.production` are **not** matched by `finsight-frontend/.gitignore` (it lists `*.local` but not `.env` / `.env.*`), so they would be committed once a git repo exists.
- Files: `finsight-frontend/.gitignore`; `finsight-frontend/.env.development`, `finsight-frontend/.env.production`.
...
- Files: `finsight-backend/src/main/java/com/lcs/finsight/services/FinancialTransactionService.java` (`importFromNubankCsv`, lines 140-192).
...
- Residual gap: most other feature components/hooks under `finsight-frontend/src/features/**` and `src/api/**` still have no dedicated tests (only the two forms and one service hook above). Coverage is real but narrow — extend incrementally as those areas change.
...
**`finsight-backend/README.md` contains multiple inaccuracies:**

- Issue: The README misstates the actual stack/config, reducing trust in project docs.
- Files & specifics:
  - **Java version:** README says "Java 21"; `finsight-backend/pom.xml` sets `java.version=17` and the `Dockerfile` builds/runs on temurin-17. Impact: **LOW** — reconcile and pin one toolchain (align README + `pom.xml` + Docker base images).
