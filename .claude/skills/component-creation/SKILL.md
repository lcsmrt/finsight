...
description: Create or modify a React component. Covers a strict component anatomy (arrow function, named export, module-scope prop type), shared vs feature-level scope, cva style variants, compound components, hook/sub-component extraction decisions, discriminated-union overlay state, and Sheet/Dialog/Popover/Page container choice. Use when writing or refactoring any .tsx component, a badge/card/table piece, a modal or drawer, or deciding where a component belongs. Triggers on "react component", "create a component", "tsx", "cva variant", "compound component", "sheet", "dialog", "drawer", "overlay state", "extract a hook", "shared vs feature component".
metadata:
  type: reference
---
...
> `cn()` is the project's class-merge helper (clsx + tailwind-merge). Import it from wherever
> your project keeps it (e.g. `@/lib/utils`). Adapt paths below to your project.
...
- `cn()` for class composition — never string concatenation or template literals
...
Group shared components by name into subfolders (`src/components/button/`, `badge/`, `card/`,
`table/`, `input/base/`, …) rather than a single flat directory.
...
Do not modify or replicate primitives from your UI library (e.g. shadcn's Button, Input, Dialog, Sheet, Popover, Select, Badge).
...
export const useTable = () => {
  ...
  if (!ctx) throw new Error("useTable must be used within a Table");
  return ctx;
};
...
const { filters, chips, pagination, queryParams } = useListFilters();
...
Before creating a modal/overlay, pick the right container. This is a spectrum guided by the
form's size, complexity, and flow — not a rigid CRUD-vs-not rule. A one-field rename form and a
12-field multi-step wizard are both "CRUD", but they don't belong in the same container:

| Container   | When to use                                                      |
| ----------- | ---------------------------------------------------------------- |
| **Dialog**  | Confirmations, small auxiliary actions, and small create/edit forms (a couple of fields, no nested data) |
...
Judge by the form itself — field count, whether it has nested/related data, whether the flow has
steps — not by whether the action is technically create/update/delete. A single-field create
dialog (e.g. "create plan": just a name) is correct as a `Dialog`; don't flag it as a skill
violation just because it handles a CRUD action.
